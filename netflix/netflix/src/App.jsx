  ...
  useEffect(() => {
    ...
    setFilt(
      !x
        ...
        : movies.filter(
            (m) =>
              m.title.toLowerCase().includes(x) || m.year.toString().includes(x)
          )
    );
  }, [q]);
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 16 }}>
              ...
              <p>{sel.overview}</p>
              <button onClick={() => alert("Play")} style={{ marginRight: 8 }}>
                Play
              </button>
              <button onClick={() => alert("Added to list")}>+ My List</button>
            </div>
          </div>
        </div>
      )}
